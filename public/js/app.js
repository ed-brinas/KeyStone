document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const loginResponse = await apiFetch(`${API_BASE_URL}/v1/login`, { method: 'POST', body: data });
        ...
        try {
            ...
            if (config.optionalGroupsForStandard && config.optionalGroupsForStandard.length > 0 && standardGroupsContainer && standardGroupsList) {
                ...
                standardGroupsList.innerHTML = config.optionalGroupsForStandard.map(g => {
                    ...
                    console.log("Checking against config group:", gn.toLowerCase()); // For debugging
        ...
        try {
            ...
            switch(action) {
                case 'reset-pw':
                    ...
                    break;
                case 'reset-admin-pw':
                    result = await apiFetch(`${API_BASE_URL}/v1/passwords/reset-admin`, { method: 'POST', body });
                    clearAlert(resetPwAlert);
                    const resetAdminPwResultUsername = document.getElementById('reset-pw-result-username');
                    const resetAdminPwResultNew = document.getElementById('reset-pw-result-new');
                    if (resetAdminPwResultUsername) resetAdminPwResultUsername.textContent = `${sam}-a`;
                    if (resetAdminPwResultNew) resetAdminPwResultNew.value = result.newPassword;
                    if (resetPasswordResultModal) resetPasswordResultModal.show();
                    break;
                case 'unlock':
